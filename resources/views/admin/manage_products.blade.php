<x-admin-master>
    @section('sidenav')
        <x-sidenav link="manage_products">
        </x-sidenav>
    @endsection
                        ...
                        <div class="card my-4">
                            ...
                            <div class="card-body px-0 pb-2">
                                <div class="table-responsive p-0">
                                    <button type="button" class="btn btn-outline-success mx-3" data-bs-toggle="modal" data-bs-target="#addProductModal">
                                            ...
                                            <tr>
                                                <td>
                                                    <div class="d-flex px-2">
                                                        <div>
                                                            <img src="{{asset('img/'.$product->image)}}" class="avatar avatar-xl rounded-circle me-2" alt="spotify">
                                                        </div>
                                                        <div class="my-auto">
                                                ...
                                                <td>
                                                    <span class="text-sm font-weight-bold">{{$product->productCategory->name}}</span>
                                                </td>
                                                <td>
                                                    <span class="text-xs font-weight-bold">TP {{$product->price}}</span>
                                                </td>
                                                <td>
                                                    <span class="text-xs font-weight-bold">{{$product->description}}</span>
                                                </td>
                                                <td class="text-center">
                                                    <form method="POST" action="{{action([\App\Http\Controllers\ProductController::class, 'destroy'], ['product' => $product])}}">
                                                        @csrf
                                                        @method('delete')
                    ...
                    <div class="modal-content">
                        <div class="modal-body p-0">
                            <div class="card card-plain">
                                ...
                                <form id="addProductForm" method="POST" action="{{action([\App\Http\Controllers\ProductController::class, 'store'])}}" enctype="multipart/form-data">
                                    @csrf
                ...
                $(function(){
                    ...
                    $('#addProductForm').on('submit', function(e){
                        ...
                        if(formData.get('description') == ''){
                            e.preventDefault();
                            $(form).find('span.description_error').text("Description cannot be empty");
                        }
                        if(! $.isNumeric(formData.get('price'))){
                            e.preventDefault();
                            $(form).find('span.price_error').text("Price must be a number");
                        }
                        if(formData.get('price') == ''){
                            e.preventDefault();
                            $(form).find('span.price_error').text("Price cannot be empty");
                        }
